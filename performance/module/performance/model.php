...
class performanceModel extends model
{
	 ...
	 */
     public function getTotalScore($staffcode,$zhouqi)
     {
     	...
     	$tt = $this->dao->select('*')->from('zt_performanceitem')->where('mid')->eq($basic->id)->andWhere('weight')->gt(0)->andwhere("zhouqi")->eq($zhouqi)->fetchAll();
        foreach($tt as $k=>$v)
        {
        ...
        $averaged = round(($itemtotal*0.7 + $abilitytotal*0.3),2);
     ...
     public function superviserreview($PID)
     {
     	...
     	{
     		$sumweight2 += intval($this->post->itemweight[$key]);
     	}
     	if($sumweight2 != 100) die(js::error("Total items weight must be equal 100 !"));
     	//save review items
