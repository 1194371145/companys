...
class fileModel extends model
{
    ...
    public function saveUpload($objectType = '', $objectID = '', $extra = '', $filesName = 'files', $labelsName = 'labels' )
    {
        ...
        foreach($files as $id => $file)
        {
            ...
            $existfile = $this->dao->select('*')->from('zt_file')->where('originalname')->eq($file['originalname'])->andWhere('size')->eq($file['size'])->orderby('id desc')->fetch();
            if(intval($existfile->id) > 0)
            {
                $file['pathname'] = $existfile->pathname;
            }
            else
            {
     ...
     * @return array
     */
    public function saveContractUpload($objectType = '', $objectID = '', $extra = '', $filesName = 'files', $labelsName = 'labels' , $control = '',$status = '')
    {
        $fileTitles = array();
        $now        = helper::today();
        $files      = $this->getUpload($filesName, $labelsName);
        foreach($files as $id => $file)
        {
            ...
            if($status == 'edit') // 编辑
            {
                $date = $this->dao->select('pathname')->from('zt_file')->where('objectType')->eq('contract')->andwhere('objectID')->eq($objectID)->fetch();
                $date = substr($date->pathname,0,strpos($date->pathname,'/')).'/'; // 得到第一次上传的年月
                if (!move_uploaded_file($file['tmpname'], $this->app->getAppRoot() . "module/contract/upload/" . $date . $control .'/' . substr($file['pathname'],strpos($file['pathname'], '/')))) return false;
                $this->savePathContract = dirname($this->app->getAppRoot() . "module/contract/upload/" . $date . $control .'/') . '/';
                $file = $this->compressImageContract($file,$objectID);
            }
            else
            {
                if (!move_uploaded_file($file['tmpname'], $this->setSaveContractPath($control) . substr($file['pathname'],strpos($file['pathname'], '/')))) return false;
                $file = $this->compressImage($file);
            }
            $file['objectType'] = $objectType;
            $file['objectID']   = $objectID;
    ...
    public function getUpload($htmlTagName = 'files', $labelsName = 'labels')
    {
        ...
        else
        {
            ...
            $file['originalname']      = substr($name, 0, strpos($name, $file['extension']) - 1);
    ...
    /**
     * Set save contract path.
     * 
     * @access public
     * @return void
     */
    public function setSaveContractPath($control)
    { 
        $savePath = $this->app->getAppRoot() . "module/contract/upload/" . date('Ym/', $this->now) . $control .'/';
        ...
        return $savePath;
        // $this->setSaveContractPath = dirname($savePath) . '/';
    }
    ...
    /**
     * Compress image Contract
     * 
     * @param  array    $file 
     * @access public
     * @return array
     */
    public function compressImageContract($file)
    {
        if(!extension_loaded('gd') or !function_exists('imagecreatefromjpeg')) return $file;

        $pathName    = $file['pathname'];
        $fileName    = $this->savePathContract . $pathName;
        $suffix      = strrchr($fileName, '.');
        $lowerSuffix = strtolower($suffix);
