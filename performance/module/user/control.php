...
class user extends control
{
    ...
    public function edit($userID)
    {
        ...
        $superarr = $this->dao->select('account,realname')->from('zt_user')->groupby('realname')->orderby('realname')->fetchPairs('account','realname');
        ...
        $this->view->addressgroup = $this->config->user->addressgroup;
        $this->view->superarr = $superarr;
        $this->display();
    }
    ...
    public function logout($referer = 0)
    {
        ...
        //session_destroy();
        unset($this->session->user->account);
    ...
    /*
     * CRM系统中 给FAE和Salse通知变更IP地址的邮件
     */
    public function sendmail()
    {
    	$thk = $this->dao->select('*')->from('zt_user')->where('dept')->in('FAE,Sales')->andwhere('deleted')->eq(0)->fetchAll();
    	
    	foreach($thk as $k=>$v)
    	{   
    		$str= "Hi &nbsp;".$v->realname."&nbsp;:<br/>
    		      <p>The IP address of CRM system has been changed from <a href='http://101.68.73.134:8001/'>http://101.68.73.134:8001</a> to <a href='https://crm.silergycorp.com:8001/'>https://crm.silergycorp.com:8001</a> .
    		      <br/>
    		      The account password will remain changed.
    		      <br/>
    		      If you have any questions, please contact IT:<a href='mailto:software@silergycorp.com'>software@silergycorp.com</a></p> ";
    		if(!empty($v->email))
    		{
    		$this->loadModel('mail')->send($v->account,"CRM system IP address change notification",$str,"admin",true);
    		}
    	}//die;
    }
