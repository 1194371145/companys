...
class userModel extends model
{
     ...
     * @return array
     */
    public function getRealNameAndEmails2($email)
    {
        //query from 510_user
        $users510 = $this->dao->select('firstname, username, lastname')->from('510_user')->where('username')->eq($email)->fetchAll();
        if(!$users510) return array();
        
        foreach($users510 as $user)
        { 
        	$realname = $user->lastname."".$user->firstname;
        	$users[$realname]->realname = $realname;   
        	$users[$realname]->email = 'gaochao.wu@silergycorp.com';
        	$users[$realname]->account = $realname;
        } 
    ...
    public function update($userID)
    {
        ...
        $userID = $oldUser->id;
        $supervise = $this->dao->select('realname')->from('zt_user')->where('account')->eq($this->post->supersid)->fetch('realname');
        $user = fixer::input('post')
            ->setDefault('join', '0000-00-00')
            ->setIF($this->post->password1 != false, 'password', md5($this->post->password1))
            ->remove('password1, password2, groups,address,verifyPassword')
            ->add('address',implode(',',$this->post->address))
            ->add('supervise',$supervise)
            ->get();
        ...
        $this->dao->update(TABLE_USER)->data($user)
            ...
            ->exec();
        //update performance superviser and supersid when change superviser in user 
        if($oldUser->supersid != $this->post->supersid || 1 == 1)
        {
           $opencircle = $this->dao->select('circle')->from('zt_circletime')->where('status')->eq('open')->fetchPairs('','circle');
           $this->dao->update('zt_performancemaster')->data(array('zgsid'=>$this->post->supersid,'zgname'=>$supervise))
                ->where('staffcode')->eq($oldUser->account)
                ->andWhere('zhouqi')->in($opencircle)
                ->exec();
        }            
    ...
    {
        return ($this->session->user->id and $this->session->user->account != 'guest');
    }
