...
class mailModel extends model
{
    ...
    public function setMTA()
    {
        ...
        $this->mta->CharSet = 'utf-8';//$this->config->encoding;
     ...
     * @return void
     */
    public function send($toList, $subject, $body = '', $ccList = '', $includeMe = false,$attr = '')
    {
        ...
        try 
        {   
            $this->mta->setFrom("webmaster@silergycorp.com", "webmaster");
            $this->setSubject($subject);
            $this->setTO($toList, $emails);
            $this->setCC($ccList, $emails);
			if(strlen($attr) > 0)
			{
			   $this->mta->AddAttachment($attr);
			}
            $this->setBody($body);
            $this->setErrorLang();
            $this->mta->send();
