...
public function ajaxgetjoindate()
	...
	$result = $this->dao->select('*')->from('zt_user')
     ...
     public function superviserreview($PID)
     {
        ...
        $get_user = $this->dao->select('*')->from('zt_user')->where('account')->eq($get_master->staffcode)->fetch();
     	...
     	if(!empty($_POST))
     	{
     		...
     		{
     			//save review item's weight
     			$masterinfo = $this->performance->getById($PID);
     			$sumweight = 0;
     			foreach($this->post->itemweight as $key => $value)
     			{
     				$sumweight += intval($this->post->itemweight[$key]);
     			}
     			if($sumweight != 100) die(js::error("Total items weight must be equal 100 !"));
     			foreach($this->post->itemweight as $key => $value)
     			{
     	           if((validater::checkInt($value) || $value == '0') && $value >= 0 && $value <= 100)
     	           {

     	   	          $itemdata['weight'] = intval($this->post->itemweight[$key])/100;
     	   	          $olditem = $this->dao->select('weight')->from('zt_performanceitem')
     	   	          ->where('mid')->eq($PID)
     	   	          ->andWhere('staffcode')->eq($masterinfo->staffcode)
     	   	          ->andWhere('zhouqi')->eq($masterinfo->zhouqi)
     	   	          ->andWhere('category')->eq($key)
     	   	          ->fetch();
     	   	          if($itemdata['weight'] != $olditem->weight)
     	   	          {
     	   	          	...
     	   	          	if($change_item)
     	   	          	{
     	   	          		...
     	   	          		$this->action->logHistory($actionID, $change_item);
     	   	          	}
     	   	          	echo js::alert('Update weight success !');
     	   	          	if(dao::isError()) die(js::error(dao::getError()));
     	   	          }
     	           }
     	           else
     	           {
     	           	   die(js::error("weight must integer(0-100) "));
     	           }
     			}
     			die(js::error('You can not do any operation before team member Self Evaluation except update weight !'));
     		}     		
      ...
      * @return void
      */
     public function exports($zhouqi)
     {  //print_r($zhouqi);die;
     ...
     {  //print_r($zhouqi);die;
		...
		$alls = $this->dao->select('*')->from('zt_user')->where('deleted')->eq(0)->fetchAll();
		foreach($alls as $kk=>$vv)
		{
			$geren = $this->dao->select('*')->from('zt_performancemaster')->where('zhouqi')->eq($zhouqi)->andwhere('staffcode')->eq($vv->account)->fetch();
			
			$totals = $this->loadModel('performance')->getTotalScore($geren->staffcode,$zhouqi);
			if($geren)
			{
				
			$re[$vv->id]['id']=$geren->id;
			$re[$vv->id]['staffcode']=$geren->staffcode;
			$re[$vv->id]['zhouqi']=$geren->zhouqi;
			$re[$vv->id]['name']=$geren->name;
			$re[$vv->id]['position']=$vv->position;
			$re[$vv->id]['zgstaffcode']=$vv->supersid;
			$re[$vv->id]['zgname']=$vv->supervise;
			$re[$vv->id]['statement']=$geren->statement;
			$re[$vv->id]['review_strength']=$geren->review_strength;
			$re[$vv->id]['review_improve']=$geren->review_improve;
			$re[$vv->id]['total']=$totals->total;
			$re[$vv->id]['status']=$geren->status;		
			}
			else
			{
			$re[$vv->id]['id']="";
			$re[$vv->id]['staffcode']=$vv->account;
			$re[$vv->id]['zhouqi']="";
			$re[$vv->id]['name']=$vv->realname;
			$re[$vv->id]['position']=$vv->position;
			$re[$vv->id]['zgstaffcode']=$vv->supersid;
			$re[$vv->id]['zgname']=$vv->supervise;
			$re[$vv->id]['statement']="";
			$re[$vv->id]['review_strength']="";
			$re[$vv->id]['review_improve']="";
			$re[$vv->id]['total']="";
			$re[$vv->id]['status']="";	
			} 
     ...
     public function close($PID,$confirm="no")
     {
        ...
        else 
        {
          ...
          $this->loadModel('mail')->send($masterinfo->staffcode, "You performance items was closed by your supervisor .", "Hi {$masterinfo->name} <br/> You performance items was closed by your supervisor,Please check .", '',false,'');
        }
    ...
    { 
		...
		{   
			...
			{   
				...
				$A=trim($ob->getActiveSheet()->getCell("A1")->getValue());//var_dump($A);
				$B=trim($ob->getActiveSheet()->getCell("B2")->getValue());//var_dump($B);
				...
				$feild=array("A"=>"account","B"=>"join","C"=>"tpdate");      
				for($row=2; $row<=$allrows; $row++)
					...
					$data['join']=$this->excelTime($data['join']);
					$data['tpdate']=$this->excelTime($data['tpdate']);
					$yu = $this->dao->select('*')->from('zt_user')->where('account')->eq($data['account'])/*->andwhere('join')->eq($data['join'])*/->fetch();
					if($yu)
					{
                    $this->dao->update('zt_user')->set('tpdate')->eq($data['tpdate'])->where('account')->eq($yu->account)->exec();
					}                 
		     }
