...
<form action="" method='post' target='hiddenwin' enctype="multipart/form-data">
		<!-- 查看附件按钮 -->
		<!-- <div class="dropdown" style='float:right;margin-top:20px;margin-right: 20px;'>
		  ...
		  <ul class="dropdown-menu" aria-labelledby="dropdownMenu1" style='width:100%;'>
		     ...
		     <li><?php echo html::a($this->inLink('viewfile',"fileid=$v->id"),"第".($k+1)."版",'_blank','') ?></li>
		  	<?php } ?>
		    <?php if (!empty($filearr2)) { ?>
	      	<li role="separator" class="divider"></li>
	      	<?php foreach ($filearr2 as $key => $value) { ?>
	      	<li><?php echo html::a($this->inLink('viewfile',"fileid=$value->id"),"法务修订".($key+1)."版",'_blank','');
	      	?></li>
	      	<?php } ?>
	      	<?php } ?>
		    <?php if (!empty($data->lagalapprove)) { ?>
	  	   <li role="separator" class="divider"></li>
	  		<li><?php echo html::a($this->inLink('viewfile',"fileid=".end($fileHis)->id),"最终版",'_blank','');
	  	   ?></li>
	      	<?php } ?>
		  </ul>
		</div> <!-- 下拉area END -->
...
<div id='main'>
	...
	<table style='border:2px solid #000;overflow:hidden;margin-top:5px;' border-collapse="collapse" align="center" class="table table-bordered">
		<tr>
			...
			<td><?php echo html::select('contract_dept',$this->config->contract->dept,$data->dept,"disabled class='form-control' autocomplete='off'"); ?></td>
			<th>部门主管<br />Dept. Manager</th>
			<td><?php echo html::select('contract_manager',$this->config->contract->select[$data->dept],$data->manager,"disabled class='form-control' autocomplete='off'"); ?></td>
			<th>合约对象<br/>Contract Party</th>
			<td><input type="text" name='contract_party' value="<?php echo $data->party ?>" disabled class='form-control'></td>
		</tr>
		<tr>
			<th>合约名称<br/>Contract Title</th>
			<td colspan="3"><input type="text" name='contract_title' value="<?php echo $data->title ?>" disabled class='form-control'></td>

			<th>文件页数<br/>Number of Pages</th>
			<td><input type="text" name='contract_number' value="<?php echo $data->pager ?>" disabled class='form-control'></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
				<label><input name="contract_object" type="radio" value="group" <?php if($data->object == 'group') echo ' checked';?> disabled/> 集团(group): </label><br/>
				<label><input name="contract_object" type="radio" value="company" <?php if($data->object == 'company') echo ' checked';?> disabled/> 个别公司(company): </label>
			</td>
			<td><textarea name="contract_maincontent" id="" cols="25" rows="5"  disabled class='form-control'><?php echo $data->maincontent;?></textarea></td>
			<td><textarea name="contract_clause" id="" cols="25" rows="5"  disabled class='form-control'><?php echo $data->clause;?></textarea></td>
			<td><label><input name="contract_close" type="radio" value="Y" <?php if($data->close == 'Y') echo ' checked';?> disabled/> 是 (Y) </label><br/>
				<label><input name="contract_close" type="radio" value="N" <?php if($data->close == 'N') echo ' checked';?> disabled/> 否 (N) </label>
			</td>
		</tr>
		<tr>
			<td colspan="4" style='height:120px;'>
				<table width="100%" style='border:0px solid #000' class='table' id='edithuishencontent'>
					<tr>
...
<!-- 			<td rowspan="2">
		...
		<tr id='table_type'>
			...
			<td colspan="4">
				<table class='table' width="100%" style='border:0px solid #000' id='edithuishentype'>
					<tr>
	...
	<table class='table table-bordered' width='100%' style='border:2px solid #000;overflow:hidden;margin-top:-2px;' border-collapse="collapse" align="center" id=''>
		<tr>
			<th>顺序<br/>order</th>
		...
		<tr>
			...
			<td colspan='2'>
				<label><input name="manager_approve[<?php echo $i;?>]" type="radio" value="1"  <?php if($statu[$i] !== $this->app->user->account) echo 'disabled';if($comments[$i-1]->approve == '1') echo ' checked';?>/>同意</label>
				<label><input name="manager_approve[<?php echo $i;?>]" type="radio" value="2"  <?php if($statu[$i] !== $this->app->user->account) echo 'disabled';if($comments[$i-1]->approve == '2') echo ' checked';?>/>需改进</label>
				<label><input name="manager_approve[<?php echo $i;?>]" type="radio" value="3" <?php if($statu[$i] !== $this->app->user->account) echo 'disabled';if($comments[$i-1]->approve == '3') echo ' checked';?> />拒绝</label>
				<textarea name="comments" id="" rows="3" style="width:65%;" <?php if($statu[$i] !== $this->app->user->account) echo 'disabled';?>><?php if(!empty($comments[$i-1]->comments)) echo $comments[$i-1]->comments;?></textarea>
			</td>
			<th><?php echo $i + 5;if($comments[$i+4]->approvedept){echo ' ('.$comments[$i+4]->approvedept.')';}?></th>
			<td colspan='2'>
				<label><input name="manager_approve[<?php echo $i+5; ?>]" type="radio" value="1" <?php if($statu[$i + 5] !== $this->app->user->account) echo 'disabled';if($comments[$i+4]->approve == '1') echo ' checked';?> /> 同意</label>
				<label><input name="manager_approve[<?php echo $i+5; ?>]" type="radio" value="2" <?php if($statu[$i + 5] !== $this->app->user->account) echo 'disabled';if($comments[$i+4]->approve == '2') echo ' checked';?> /> 需改进</label>
				<label><input name="manager_approve[<?php echo $i+5; ?>]" type="radio" value="3" <?php if($statu[$i + 5] !== $this->app->user->account) echo 'disabled';if($comments[$i+4]->approve == '3') echo ' checked';?> /> 拒绝</label>
				<textarea name="comments" id="" rows="3" style="width:65%;" <?php if($statu[$i + 5] !== $this->app->user->account) echo 'disabled';?>><?php if(!empty($comments[$i+4]->comments)) echo $comments[$i+4]->comments;?></textarea></td>
		</tr>
		<?php } ?>
		<tr>
			<th colspan='1'>备注说明<br/>Remark</th>
			<td colspan='5'>
				<textarea name="contractself_remark" id="" cols="20" rows="3" class='form-control' style="resize:none;" disabled="disabled"><?php echo $data->remark; ?></textarea>
			</td>
		</tr>
		<tr>
		...
		<input type="hidden" name='editselfid' value="<?php echo $data->id; ?>">
	</table>
...
<script>
	$(function(){
		$('#edithuishencontent').find("input,select").attr("disabled", "disabled"); // 禁用会审table的input textarea 
		$('#edithuishentype').find("input,textarea").attr("disabled", "disabled"); // 禁用table的input textarea 
