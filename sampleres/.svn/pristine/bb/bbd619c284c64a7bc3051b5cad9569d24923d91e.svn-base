...
class commonModel extends model
{
    ...
    public static function printTopBar()
    {
        ...
        if(isset($app->user))
        {
            ...
            if(!$isGuest)
            {
                if ($app->getModuleName() == 'sampleapp') 
                {
                    echo '<li>' . html::a(helper::createLink('my', 'profile', '', '', true), 'Personal file', '', "class='iframe' data-width='600'") . '</li>';
                    echo '<li>' . html::a(helper::createLink('my', 'changepassword', '', '', true), 'ChangePassword', '', "class='iframe' data-width='500'") . '</li>';
                } else {
            ...
            else
            {
                if ($app->getModuleName() == 'sampleapp') 
                {
                    echo html::a(helper::createLink('user', 'logout'), 'Logout');
                } else {
        ...
        if ($app->getModuleName() == 'sampleapp') 
        {
           if($app->company->website)  echo html::a($app->company->website,  'Official website',  '_blank');
        ...
        echo html::a("http://192.168.5.16:8089/index.php", "Prorelease", '_blank');    
    ...
    public static function printBreadMenu($moduleName, $position)
    {
        ...
        $mainMenu = $moduleName;
        if(isset($lang->menugroup->$moduleName)) $mainMenu = $lang->menugroup->$moduleName;
        echo html::a(helper::createLink('company', 'index'), 'Silergy') . $lang->arrow;
        if($moduleName != 'index')
        {
