...
#anniu:visited{background-color:white;}
</style>
...
<form action="" method='post' target="_blank" name="myform">
	<table style="width:100%;">
		<tr>
			...
			<td style="width:80px"><input type='submit'  value="批量出货" style="padding:0.8px;width:70px" onclick="batchuhuo()" ;/></td>
			<?php
			if (common::hasPriv("sampleout","batchPrint")) {
			echo "<td><input type='submit'  value='批量打印' style='padding:0.8px;width:70px' onclick='batchPrint()';/></td>";
			}
			?>
		</tr>
	</table>
...
<script type="text/javascript">
    function batchuhuo(){
      document.myform.action="<?php echo $this->createLink('sampleout','batchuhuo');?>";
...
<th><?php common::printOrderLink("close", $orderBy, $vars, "状态");?></th>
...
<?php if($v->close=='wait'){?>
		...
		<tr>
		<td onclick='addsampleid("<?php echo $v->id;?>");'><?php if($v->approve==0 || $v->approve=='2'){?><input type='checkbox' value='<?php echo $v->id;?>' id="formid<?php echo $v->id?>" onclick='addsampleid("<?php echo $v->id;?>");'/><?php }?>
		</td>
		...
		<td title="<?php echo $notpart[trim($v->partn)];?>"><input type='text' name='partn' value="<?php echo trim($v->partn);?>" class='input' title="<?php echo $notpart[trim($v->partn)];?>"/></td>
		...
		echo html::select("mid",$v->wait,$mid,"class = 'mid{$v->id} input'");//生成input标签
		?>
		</td>
		<td class='my_sta'><?php
		 if ($v->close == 'wait') 
		 {
		 	if ($v->approve=='1' or $v->approve=='3') 
		 	{
		 		...
		 		{
		 			echo "<b style='color:red;'>待区域经理审核</b>";		
		 		}
		 		if ($v->areamanager=='1' and empty($v->salesmanager)) 
		 		{
		 			echo "<b style='color:red;'>已被区域经理拒绝</b>";
		 		}
		 		if ($v->areamanager=='2' and empty($v->salesmanager)) 
		 		{
		 			echo "<b style='color:red;'>等待lynn审核</b>";
		 		}
		 		if ($v->areamanager=='2' and $v->salesmanager=='1') 
		 		{
		 			echo "<b style='color:red;'>已被lynn拒绝</b>";				
		 		}
		 		if ($v->areamanager=='2' and $v->salesmanager=='2') 
		 		{
		 			echo "<b style='color:green'>审核已通过可出货</b>";
		 		}
		 	}
		 	if ($v->approve=='0') {
		 		//echo $lang->sampleout->close[$v->close];
		 		echo "<b style='color:black'>申请中</b>";
		...
		<?php if(($v->area == 'NC' or $v->area == 'SC') and ($v->approve == '0' or $v->approve == '2')) {?>
		<?php 
		// 判断打印权限
		if (common::hasPriv("sampleout","batchPrint")) {
			echo html::a($this->createLink('sampleout','batchPrint',"id=$v->id"),'打印地址',"_blank"," id =anniu style='background-color:#1A4F85;color:white;padding:1.5px;border:none;font-size:10px;margin:6px 0px 6px 0px;float:left';");
		}	
		?>
		<?php }?>
		...
		?>
		<?php }else{ if(!in_array($this->app->user->account,array_keys($toassign))){?>
...
<?php 	}else{ ?>
		...
		<?php 
				...
				elseif($v->type=='2')
				{
					...
					echo html::a($this->createLink('sample','viewsample',"id=$v->mid"),mb_substr($lang->sampleout->type[$v->type],0,2,'UTF-8').date('y/n/j',strtotime($no->date)).$no->packagetype);
				}
				elseif($v->type=='3')
				{
		...
		<td class='my_sta'><?php
		 if ($v->close == 'wait') 
		 {
		 	if ($v->approve=='1' or $v->approve=='3') 
		 	{
		 		...
		 		if ($v->areamanager=='2' and $v->salesmanager=='2') 
		 		{
		 			echo "<b style='color:green'>审核已通过待出货</b>";
		 		}
		 	}
		 	if ($v->approve=='0') {
		 		// echo $lang->sampleout->close[$v->close];
...
function addsampleid(e)
{
	...
	console.log(samplevalue,formvalue);
...
// 显示my_sta titlt
$('.my_sta').on('mouseover',function(){
	$(this).attr('title',$(this).children('b').html());
});
$('.my_sta').on('mouseout',function(){
	$(this).attr('title','');
});
</script>
