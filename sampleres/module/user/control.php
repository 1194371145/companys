    ...
    public function login($referer = '', $from = '')
    {
        ...
        if($this->user->isLogon())
        {
            ...
            if(strpos($this->referer, $loginLink) === false and 
               strpos($this->referer, $denyLink)  === false  and !isset($_GET['account']) and !isset($_GET['password']) and !empty($referer)
            )
            {
            ...
            if($user)
            {
                ...
                )
                {
                    ...
                    else
                    {
                        ...
                        die(js::locate($this->createLink($this->config->default->module,$this->config->default->method), 'parent'));//登录用户没权限,重定向到sampleapp模块的getsamplelist方法  (config下的config.php文件下)
                    }
                }
                else
                {
                    ...
                    die(js::locate($this->createLink($this->config->default->module,$this->config->default->method), 'parent'));
                }
            }
            else
            {
