...
class project extends control
{
    ...
    public function downloadpdf()
	{
     /**
      ...
      $list = $this->dao->select('title,pic')->from('content_cn')->fetchAll();
	foreach($list as $value)
	{
      $link = trim($value->pic);
	  $pro = explode(':',trim($value->title));
	  $proname = $pro[0];
	  $tmparr1 = explode('/',$link);
	  $tmparr2 = explode('?',$tmparr1[count($tmparr1)-1]);
	  $dirname = $tmparr1[count($tmparr1)-2];
	  $filename = "./pic/".$proname."-".$tmparr2[0];
	  
	  if(!file_exists($filename) && strlen($link) > 1)
	  {
      ob_start();
      readfile($link);
