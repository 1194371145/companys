...
//        print_r($updates);die;
    ...
    public function mpUpdate($data, $field, $params = [])
    {
        if (!is_array($data) || !$field || !is_array($params)) {
            return false;
        }
        ...
        $sql = sprintf("UPDATE `%s` SET %s WHERE `%s` IN (%s) %s", 'zt_mp', $updates, $field, $fields, $where);
        return $sql;
    }
    public function mpiUpdate($data, $field, $params = [])
    {
        if (!is_array($data) || !$field || !is_array($params)) {
            return false;
        }
        ...
        $sql = sprintf("UPDATE `%s` SET %s WHERE `%s` IN (%s) %s", 'zt_mpi', $updates, $field, $fields, $where);
        return $sql;
    }
    public function sampleUpdate($data, $field, $params = [])
    {
        if (!is_array($data) || !$field || !is_array($params)) {
            return false;
        }
        ...
        $sql = sprintf("UPDATE `%s` SET %s WHERE `%s` IN (%s) %s", 'zt_sample', $updates, $field, $fields, $where);
        return $sql;
    }
